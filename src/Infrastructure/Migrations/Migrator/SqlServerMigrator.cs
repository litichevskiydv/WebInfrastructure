namespace Skeleton.Migrations.Migrator
{
    ...
    public class SqlServerMigrator : IMigrator
    {
        private readonly ISessionsFactory _sessionsFactory;
        private readonly IMigration[] _migrations;

        public SqlServerMigrator(ISessionsFactory sessionsFactory, IMigration[] migrations)
        {
...
if object_id ('[dbo].[VersionInfo]') is null
	begin
        ...
        {
            return session.Query<long>("select v.[Version] from [dbo].[VersionInfo] v").ToArray();
        }

        private static void AddAppliedVersion(ISession session, long version)
        {
            session.Execute("INSERT INTO [dbo].[VersionInfo] ([Version]) VALUES (@Version)",
                new {Version = version});
        }
        ...
        public void Migrate()
        {
            ...
            foreach (var newMigration in newMigrations)
                using (var session = _sessionsFactory.Create())
                {
                    var commands = newMigration.SqlSourceCode.Split(new[] {"\nGO\r\n"}, StringSplitOptions.RemoveEmptyEntries);
